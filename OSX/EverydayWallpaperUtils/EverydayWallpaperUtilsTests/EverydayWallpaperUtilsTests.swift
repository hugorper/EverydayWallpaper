...
class EverydayWallpaperUtilsTests: XCTestCase
{
    ...
    func testBingWallpaperDate()
    {
        ...
        do
        {
            let bing = try BingWallpaperService.GetTodayBingWallpaperReference(DefaultMarket); XCTAssert(bing?.StartDate.compare(fiveDaysBefore) == NSComparisonResult.OrderedDescending, "Invalid date")
        }
    ...
    func testAllMarkets()
    {
        do
        {
            ...
            {
                let bing = try BingWallpaperService.GetTodayBingWallpaperReference(market.rawValue); XCTAssert(bing?.Url.characters.count > 0, "Market wallpaper \(market) not found")
            }
        }
        catch DownloadStatus.NetworkNotReachable
        {
